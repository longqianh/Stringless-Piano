...
/***********************插入图标*********************/ 
void DISPIcon(uchar x,uchar y,uchar clong,uchar hight,uchar *Icon)
{
	uchar i,j;
	for(i=0;i<hight;i++)
	{
...
void set_CGRAM(uchar *zimo)
{
	uchar i;
	Write_Cmd(0x34);
	Write_Cmd(0x02); //允许设置IRAM和CGRAM地址
	Write_Cmd(0x30);
	//Write_Cmd(0x40+(num<<4)); 
	Write_Cmd(0x40);
	for(i=0;i<16;i++)
	{
		Write_Data(*zimo++);
